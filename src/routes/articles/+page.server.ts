...
export const load = (async ({ locals, url, params }) => {
  ...
  const all = await content.getEntries<TypeArticleSkeleton>({ content_type: "article", include: 1, select: ["fields.categorie"], "fields.date[exists]": true, order: ["-fields.date"], limit: 200, locale: { en: 'en-CA' }[languageTag()] || 'fr-CA' })
  ...
  const [vedettes, articles] = await Promise.all([
    content.getEntries<TypeArticleSkeleton>({ content_type: "article", include: 2, "fields.vedette": true, order: ["-fields.date"], limit: 3, ...filter ? { links_to_entry: categories[filter].sys } : {}, locale: { en: 'en-CA' }[languageTag()] || 'fr-CA' }),
    content.getEntries<TypeArticleSkeleton>({ content_type: "article", include: 2, order: ["-fields.date"],  limit: 12, ...filter ? { links_to_entry: categories[filter].sys } : {}, locale: { en: 'en-CA' }[languageTag()] || 'fr-CA' }),
  ])
